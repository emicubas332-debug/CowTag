...
function SanidadModule() {
  ...
  useEffect(() => {
    setLoading(true);
    const q = query(collection(db, "sanidad"), orderBy("fechaAplicacion", "desc"));
  ...
  const guardarRegistro = async (e) => {
    ...
    try {
      if (editId) {
        const docRef = doc(db, "sanidad", editId);
        await updateDoc(docRef, form);
      } else {
        await addDoc(collection(db, "sanidad"), form);
      }
      limpiarForm();
    } catch (err) {
  ...
  const eliminarRegistro = async (id) => {
    if (window.confirm("¿Está seguro de eliminar este registro?")) {
      try {
        await deleteDoc(doc(db, "sanidad", id));
      } catch (err) {
  ...
  return (
    ...
    >
      ...
      <form onSubmit={guardarRegistro} style={{ marginBottom: 30 }}>
        <input
          name="animalTag"
          placeholder="Tag Animal (ID)"
          value={form.animalTag}
          onChange={handleChange}
          required
          style={inputStyle}
          aria-label="Tag Animal"
        />
        <input
          name="tipo"
          placeholder="Tipo vacuna o medicamento"
          value={form.tipo}
          onChange={handleChange}
          required
          style={inputStyle}
          aria-label="Tipo vacuna o medicamento"
        />
        <input
          name="dosis"
          placeholder="Dosis"
          ...
          aria-label="Dosis"
        />
        <label style={{ display: "block", marginBottom: 8, marginTop: 8, color: "#004d40" }}>
          Fecha de Aplicación*
        </label>
        <input
          ...
          onChange={handleChange}
          required
          style={{ ...inputStyle, padding: "10px 12px", marginBottom: 12 }}
          aria-label="Fecha de aplicación"
        />
        <label style={{ display: "block", marginBottom: 8, color: "#004d40" }}>
        ...
        <input
          ...
          onChange={handleChange}
          style={inputStyle}
          aria-label="Próxima aplicación"
        />

        {error && (
          <p role="alert" style={{ color: "red", marginTop: 10, marginBottom: 10 }}>
            {error}
          </p>
        )}
        ...
        <div style={{ display: "flex", gap: 16 }}>
          <button
            ...
            disabled={saving}
            style={{
              backgroundColor: saving ? "#004d40" : "#00796b",
              color: "white",
              padding: "12px 24px",
              border: "none",
              borderRadius: 8,
            ...
            aria-label={editId ? "Actualizar registro" : "Registrar registro"}
          >
      ...
      {loading ? (
        ...
        <table
          ...
          aria-label="Tabla de registros de sanidad"
        >
          ...
          <tbody>
            {registros.map((r, i) => (
              <tr
                key={r.id}
                style={{
                  backgroundColor: i % 2 === 0 ? "#e0f2f1" : "white",
              ...
              >
                ...
                <td style={{ ...tdStyle, display: "flex", gap: 8, justifyContent: "center" }}>
                  <button
                    ...
                    style={btnEditar}
                    aria-label={`Editar registro para animal ${r.animalTag}`}
                  >
                    Editar
                  </button>
                  <button
                    ...
                    style={btnEliminar}
                    aria-label={`Eliminar registro para animal ${r.animalTag}`}
                  >
                    Eliminar
                  </button>
                </td>
